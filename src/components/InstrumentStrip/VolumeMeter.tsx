...
export const VolumeMeter: React.FC<VolumeMeterProps> = ({ size = 200 }) => {
  ...
  const rawVol = useStore((s) => (symbol ? s.volume[symbol] : undefined));
  const avgVol = useStore((s) => (symbol ? s.avgVolume[symbol] : 100));
  ...
  const targetAngle = isFinite(clamped)
    ? -90 + (clamped / 200) * 180
    : -90;
  ...
  return (
    <Panel title="Volume meter" style={{ width: "100%" }}>
      <div style={{ display: "grid", paddingTop: size * 0.1, placeItems: "center", width: "100%" }}>
        ...
        >
          <span style={{ opacity: 0.75, marginRight: 6 }}>Vol:</span>
